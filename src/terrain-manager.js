...
export class TerrainManager {
  ...
  _processBuildQueue() {
    // Nearest chunks first — ensures the player's current chunk gets physics
    // before distant ones, preventing fall-through on spawn.
    if (this._buildQueue.length > 1 && this._lastCX !== null) {
      const cx = this._lastCX, cz = this._lastCZ;
      this._buildQueue.sort((a, b) => {
        const [acx, acz] = a.key.split('_').map(Number);
        const [bcx, bcz] = b.key.split('_').map(Number);
        return (acx-cx)**2 + (acz-cz)**2 - ((bcx-cx)**2 + (bcz-cz)**2);
      });
    }
