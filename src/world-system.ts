...
import { NpcAgent } from './systems/npc-system.js';
    ...
    const spawns = [
      {
        ...
        z: 0,
        name: 'Elder Kira',
        dialogue: 'This world was shaped by ancient hands. Every ridge and valley has a story.',
      },
      {
        x: 0,
        z: 10,
        name: 'Merchant Dax',
        dialogue: 'Rare finds today. Strange things washing up from the deep terrain rifts.',
      },
      {
        x: -8,
        z: 6,
        name: 'Scout Ren',
        dialogue: 'Something large moved through the eastern sector last night. Stay alert.',
      },
      {
        x: 6,
        z: -9,
        name: 'Wanderer',
        dialogue:
          'I have walked every biome in this world. Nothing quite like the cliff edges at dusk.',
      },
    ];

    for (const s of spawns) {
      const y = getTerrainHeight(s.x, s.z) + 0.05;
      const e = this.world.createTransformEntity(undefined, {
        parent: this.world.sceneEntity,
        persistent: true,
      });
      e.object3D.position.set(s.x, y, s.z);
      e.addComponent(NpcAgent, {
        modelUrl: '/gltf/robot/robot.gltf',
        npcName: s.name,
        dialogue: s.dialogue,
      });
    }
